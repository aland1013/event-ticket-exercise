  ...
  describe('Functions', () => {
    ...
    describe('buyTickets()', async () => {
      ...
      it('buyers should be refunded any surplus funds sent with the transaction', async () => {
        ...
        const buyReceipt = await instance.buyTickets(1, {
          from: secondAccount,
          value: paymentAmount
        });
        ...
        assert.equal(
          postSaleAmount,
          new BN(preSaleAmount)
            .sub(new BN(ticketPrice))
            .sub(new BN(buyTxCost))
            .toString(),
    ...
    describe('getRefund()', async () => {
      it('buyers should be refunded the appropriate value amount when submitting a refund', async () => {
        ...
        let refundTxCost =
          Number(refundTx.gasPrice) * refundReceipt.receipt.gasUsed;

        assert.equal(
          postSaleAmount,
          new BN(preSaleAmount)
            .sub(new BN(buyTxCost))
            .sub(new BN(refundTxCost))
            .toString(),
    ...
    describe('endSale()', async () => {
      ...
      it('the contract balance should be transferred to the owner when the event is closed', async () => {
        ...
        const buyReceipt = await instance.buyTickets(numberOfTickets, {
          from: secondAccount,
        ...
        let endSaleTxCost =
          Number(endSaleTx.gasPrice) * endSaleReceipt.receipt.gasUsed;

        assert.equal(
          postSaleAmount,
          new BN(preSaleAmount)
            .add(new BN(numberOfTickets).mul(new BN(ticketPrice)))
            .sub(new BN(endSaleTxCost))
            .toString(),
